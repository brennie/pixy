...
class PixyApp(Flask):
	...
	def __init__(self, development=False):
		...
		self.add_url_rule('/profile/<int:id>', view_func=ProfileView.as_view('profile'))
		self.add_url_rule('/upload', view_func=UploadView.as_view('upload'), methods=['GET', 'POST'])
		self.add_url_rule('/image/<int:id>.jpg', view_func=RawImageView.as_view('rawImage'))
		self.add_url_rule('/image/<int:id>', view_func=ImageView.as_view('image'))
