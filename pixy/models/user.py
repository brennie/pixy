...
class User(db.Model):
    ...
    def validate_username(username):
        ...
        if not USER_RE.match(username):
            flash('The user name has to be an alphanumeric value between 2-32 '
                  'characters', 'danger')
            valid = False
    ...
    def validate_password(password):
        if not SPECIAL_RE.search(password) or not len(password) >= 8:
            flash('The password must be at least 8 characters and contain a '
                  'non-alphanumeric character', 'danger')
            return False
    ...
    def validate_user(username, email, password, confirm):
        ...
        if not User.validate_username(username) or \
           not User.validate_email(email) or \
