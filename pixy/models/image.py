...
class Image(db.Model):
    ...
    def validate_image(title, visible, description, filename, tags):
        ...
        if len(description) > 512:
            flash('Description length must be at most 512 characters',
                  'danger')
            valid = False
        ...
        if not TAG_RE.match(tags):
            flash('Tags must be 1-16 alphabetic characters seperated by '
                  'spaces', 'danger')
            valid = False
    ...
    # \param private If the image is public or private.
...
class Transform(db.Model):
    ...
    # \return The absolute path to the image.
    def path(self):
        return "{0}{1}{2}{3}".format(Transform.ROOT, Transform.PREFIX,
                                     self.name, Transform.SUFFIX)
