...
from flask import abort, flash, make_response, render_template, redirect, \
    request, session, send_file, url_for
...
class EditImageView(View):
    ...
    # \brief Handle an HTTP request.
    #
    # This method calls EditImageView.dispatch_get if the request is an HTTP
    # GET request; it passes control to EditImageView.dispatch_post if it is an
    # HTTP POST request.
    #
    ...
    # \return If given id is invalid, then a 404 error is returned. If the
    #         image is not editable by the user, then a redirect to the index
    #         is returned. Otherwise, the edit image HTML page for the image
    ...
    #         for the image page is returned. Otherwise a redirect for the
    #         index is returned.
    def dispatch_post(self, id):
        ...
        if action == 'delete':
            ...
            if not Image.validate_image(title, visible, description, i.path(),
                                        tags):
